...
export const acaoCorretivaController = {
    async criarRascunhoAcaoCorretiva(
        ...
        const acaoCorretiva = await acaoCorretivaService.criarRascunhoAcaoCorretiva(
            request.user,
            request.params.naoConformidadeId,
            request.body,
        );
        return reply.status(201).send(acaoCorretiva);
    },

    async atualizarAcaoCorretiva(
        request: FastifyRequest<{ Params: { id: string }; Body: AcaoCorretivaRascunhoInput }>,
        reply: FastifyReply,
    ) {
        const acaoCorretiva = await acaoCorretivaService.atualizarAcaoCorretiva(
            request.params.id,
            request.user,
            request.body,
        );
    ...
    async decidirAcaoCorretiva(
        ...
        const acaoCorretiva = await acaoCorretivaService.decidirAcaoCorretiva(
            request.params.id,
            request.user,
            request.body,
        );
    ...
    async finalizarExecucaoAcaoCorretiva(
        request: FastifyRequest<{ Params: { id: string }; Body: FinalizarExecucaoInput }>,
        reply: FastifyReply,
    ) {
        const acaoCorretiva = await acaoCorretivaService.finalizarExecucaoAcaoCorretiva(
            request.params.id,
            request.user,
            request.body.diasParaVerificar,
        );
        return reply.status(200).send(acaoCorretiva);
    },

    async cancelarAcaoCorretiva(
        request: FastifyRequest<{ Params: { id: string }; Body: { motivo: string } }>,
        reply: FastifyReply,
    ) {
        const acaoCorretiva = await acaoCorretivaService.cancelarAcaoCorretiva(
            request.params.id,
            request.user,
            request.body.motivo,
        );
